...
export const HelloBg = () => {
  ...
  return (
    <AnimatePresence mode='wait'>
      <motion.div ref={ref} className='top-0 left-0 w-full h-full z-0'>
        <motion.div
...
function Sections() {
  ...
  if (!collections.length) {
    return (
      <Container classes={`flex justify-center items-center h-screen relative`}>
        <Loader />
      </Container>
    );
  }
      ...
      {Object.entries(SectionIndex).map(([collectionName, index]) => (
        <li key={index}>
          ...
          <Section
            ...
            className={sectionClasses[collectionName]}
